...
<title>AnimMate | Health Records</title>
...
<style>
    :root {
        ...
        --medical-red: #ef4444;   /* For health accents */
        --medical-soft: #fee2e2;
    ...
    .card-header-custom {
        padding: 20px 25px;
        background: var(--medical-soft); /* Light Red for Health context */
    ...
    .form-control:focus { border-color: var(--medical-red); box-shadow: 0 0 0 4px var(--medical-soft); }
    ...
    @media (max-width: 991.98px) {
        ...
        /* Hide text in user profile on mobile */
        .user-profile .text-end { display: none !important; }
    }
</style>

<script>
function fillForm(id, animalId, disease, treatment, date){
    document.getElementsByName("record_id")[0].value = id;
    document.getElementsByName("animal_id")[0].value = animalId;
    document.getElementsByName("disease")[0].value = disease;
    document.getElementsByName("treatment")[0].value = treatment;
    document.getElementsByName("record_date")[0].value = date;
    
    // Highlight form visual cue
    const card = document.getElementById("formCard");
    card.style.borderColor = "#ef4444";
    setTimeout(() => { card.style.borderColor = "#fff7ed"; }, 800);
...
<div class="sidebar" id="sidebar">
    ...
    <div class="d-flex flex-column gap-2 mt-4">
        ...
        <a href="HealthRecordPanel.jsp" class="nav-link active"><i class="fas fa-heartbeat"></i> Health</a>
...
<div class="main-content">
    ...
    <div class="page-body">
        ...
        <div class="mb-4">
            <h3 class="fw-bold m-0" style="color:#b91c1c;">Health Management</h3>
            <p class="text-muted m-0">Track treatments, diseases, and check-ups.</p>
        </div>
        ...
        if(action != null){
            try {
                ...
                if(action.equals("Add")){
                    PreparedStatement ps = con.prepareStatement("INSERT INTO health_record(animal_id,disease,treatment,record_date) VALUES(?,?,?,?)");
                    ps.setInt(1,Integer.parseInt(request.getParameter("animal_id")));
                    ps.setString(2,request.getParameter("disease"));
                    ps.setString(3,request.getParameter("treatment"));
                    ps.setString(4,request.getParameter("record_date"));
                    ps.executeUpdate();
                    msg = "<i class='fas fa-check-circle me-2'></i> Health Record Added!";
                    msgClass = "success-msg";
                }

                if(action.equals("Update")){
                    PreparedStatement ps = con.prepareStatement("UPDATE health_record SET animal_id=?, disease=?, treatment=?, record_date=? WHERE record_id=?");
                    ps.setInt(1,Integer.parseInt(request.getParameter("animal_id")));
                    ps.setString(2,request.getParameter("disease"));
                    ps.setString(3,request.getParameter("treatment"));
                    ps.setString(4,request.getParameter("record_date"));
                    ps.setInt(5,Integer.parseInt(request.getParameter("record_id")));
                    ps.executeUpdate();
                    msg = "<i class='fas fa-sync-alt me-2'></i> Record Updated!";
                    msgClass = "update-msg";
                }

                if(action.equals("Delete")){
                    PreparedStatement ps = con.prepareStatement("DELETE FROM health_record WHERE record_id=?");
                    ps.setInt(1,Integer.parseInt(request.getParameter("record_id")));
                    ps.executeUpdate();
                    msg = "<i class='fas fa-trash-alt me-2'></i> Record Deleted.";
                    msgClass = "delete-msg";
                }
                con.close();
            } catch(Exception e) {
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-4 order-lg-1 order-1">
                <div class="saas-card" id="formCard">
                    <div class="card-header-custom">
                        <i class="fas fa-notes-medical text-danger fs-4"></i>
                        <h5 class="m-0 fw-bold" style="color:#991b1b;">Record Details</h5>
                    </div>
                    <div class="p-4">
                        <form method="post">
                            <div class="mb-3">
                                <label class="form-label">Record ID (Auto)</label>
                                <input type="number" name="record_id" class="form-control" placeholder="Select from list..." readonly style="background:#fef2f2;">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Animal ID</label>
                                <input type="number" name="animal_id" class="form-control" placeholder="e.g. 101" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Disease / Condition</label>
                                <input type="text" name="disease" class="form-control" placeholder="e.g. Flu, Injury" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Treatment Given</label>
                                <input type="text" name="treatment" class="form-control" placeholder="e.g. Antibiotics">
                            </div>
                            <div class="mb-4">
                            ...
                            <div class="d-grid gap-2">
                                ...
                                <div class="row g-2">
                                    ...
                                    <div class="col-6">
                                        <button class="btn-action btn-delete" type="submit" name="action" value="Delete" onclick="return confirm('Delete this medical record?');">
            ...
            <div class="col-lg-8 order-lg-2 order-2">
                <div class="saas-card">
                    <div class="card-header-custom">
                        <i class="fas fa-file-medical-alt text-danger fs-4"></i>
                        <h5 class="m-0 fw-bold" style="color:#991b1b;">Medical History</h5>
                        ...
                        <table class="table mb-0">
                            ...
                            <tbody>
                                ...
                                try{
                                    ...
                                    ResultSet rs = st.executeQuery("SELECT * FROM health_record ORDER BY record_date DESC");
                                ...
                                <tr onclick="fillForm(
                                    ...
                                    <td><span class="badge bg-danger bg-opacity-10 text-danger border border-danger">#<%= rs.getInt("record_id") %></span></td>
                                    <td class="fw-bold text-dark">ID: <%= rs.getInt("animal_id") %></td>
                                    <td><%= rs.getString("disease") %></td>
                                    <td><%= rs.getString("treatment") %></td>
                                    <td class="text-muted small"><%= rs.getDate("record_date") %></td>
                                </tr>
                                <%
                                    }
                                    con.close();
                                } catch(Exception e){
                                    out.println("<tr><td colspan='5' class='text-danger'>Error loading data: "+e.getMessage()+"</td></tr>");
                                }
                                %>
                            </tbody>
                        </table>
